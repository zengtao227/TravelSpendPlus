...
- Expense photo attachments (`docs/design.md` section 4's data model lists a `photo` field on `Expense`) — not in the domain model or schema here. Flagged by Fable's final acceptance review (2026-07-23) as a silent omission worth naming explicitly; reasonable to build alongside the UI's expense-entry form (file picker, storage path) rather than in this layer, but the UI plan should account for it as a real requirement, not treat it as newly discovered scope.

## Known non-blocking items carried forward from Fable's final acceptance review (2026-07-23)

Implementation was accepted with these fixed inline (see the plan's Task 9 section and git history for the actual fixes): a crash on `Expense(paidFor: [])` round-tripping through persistence (`''.split(',')` returns `['']`, not `[]`) — now rejected at construction, and the persistence layer parses an empty `paidForIds` defensively regardless. Also fixed: `path` package now declared explicitly in `pubspec.yaml` instead of relying on a transitive dependency.

Two items were reviewed and intentionally left as-is — not bugs, but worth the UI-phase implementer knowing about:
- `Trip.totalDays` and `BudgetCalculator.remainingDailyBudget`'s `daysLeft` use `DateTime.difference(...).inDays + 1` on local time, which is off by one across a DST transition (a Duration isn't a whole 24h that day). All current tests use January dates, so this is latent and untested. Low priority; revisit if a real trip spans a DST boundary and the daily-budget number looks off by a day's worth of spending.
- `remainingDailyBudget` excludes *today's own* actual spending from "budget used so far" (only actual expenses dated strictly before `asOf`'s day count) — money spent earlier today doesn't reduce today's remaining-daily-budget number until tomorrow. This was an implementation-time judgment call, not something design.md's worked example actually disambiguates (the example only covers day boundaries, not same-day behavior) — worth confirming with the user is the intended UX before the UI surfaces this number, not assuming it's already spec-confirmed.
