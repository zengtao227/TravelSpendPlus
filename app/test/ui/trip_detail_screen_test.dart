...
void main() {
  ...
  setUp(() async {
    ...
    photoTempDir = await Directory.systemTemp.createTemp(
      'trip_detail_screen_test',
    );
  ...
  Widget wrap(
    ...
    locale:
        locale, // most tests tap/assert Chinese labels; callers can verify other locales
  ...
  testWidgets('English budget and breakdown controls fit a phone viewport', (
    tester,
  ) async {
    final view = TestWidgetsFlutterBinding.ensureInitialized()
        .platformDispatcher
        .views
        .first;
    view.physicalSize = const Size(432, 960);
    ...
    await repo.createTrip(
      Trip(
        ...
        homeCurrency: 'EUR',
        totalBudget: Money.fromMajor(1800, 'EUR'),
        participants: [me],
      ),
    );
    await repo.addExpense(
      Expense(
        ...
        tripId: 't1',
        category: 'lodging',
        amount: Money.fromMajor(720, 'EUR'),
        amountInHomeCurrency: Money.fromMajor(720, 'EUR'),
        description: 'Hotel',
        date: DateTime.now(),
    ...
    expect(find.byKey(const Key('breakdownDimensionSwitch')), findsOneWidget);
    expect(tester.takeException(), isNull);
  });

  testWidgets('a not-yet-departed trip shows a countdown, not a daily budget', (
    tester,
  ) async {
  ...
  testWidgets('an in-progress trip shows the daily remaining budget', (
    tester,
  ) async {
    await repo.createTrip(
      Trip(
  ...
  testWidgets(
    'the average-daily-spend line reflects actual spend regardless of budget',
    (tester) async {
      await repo.createTrip(
        Trip(
          id: 't1',
  ...
  testWidgets(
    'a trip on its own last calendar day still shows the daily budget, not finished',
    (tester) async {
  ...
  testWidgets(
    'the category legend shows each category name and its exact amount, not just percentages in the pie',
    (tester) async {
      await repo.createTrip(
        Trip(
          id: 't1',
  ...
  testWidgets(
    'an expense with no stored photo shows its category icon in the list row',
    (tester) async {
      await repo.createTrip(
        Trip(
          id: 't1',
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      await tester.tap(
        find.text('地点'),
      ); // "Location" segment label, this harness's locale is zh
      await tester.pumpAndSettle();
  ...
  testWidgets(
    'an expense marked "exclude from chart" is skipped by the pie chart legend '
    'but still counted in the Actual total',
    (tester) async {
      await repo.createTrip(
        Trip(
          id: 't1',
  ...
  testWidgets(
    'an actual expense is reflected in totals and the category chart',
    (tester) async {
      await repo.createTrip(
        Trip(
          id: 't1',
  ...
  testWidgets(
    'an expense with a location shows "category · location" in its row, but a '
  ...
  testWidgets(
    'tapping "mark as spent" on a planned expense converts it to actual',
    (tester) async {
      await repo.createTrip(
        Trip(
          id: 't1',
  ...
  testWidgets(
    'tapping an expense row opens it for editing, and saving there updates it in place',
    (tester) async {
      await repo.createTrip(
        Trip(
          id: 't1',
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      expect(
        find.text('CHF 1,000.00'),
        findsOneWidget,
        reason: 'budget must still read in CHF',
      );
      reloaded = await repo.getTrip('t1');
      expect(
        reloaded!.homeCurrency,
        'CHF',
        reason:
            'editing an unrelated expense field must not revert the home currency',
      );
    },
  );
  ...
  testWidgets(
    ...
    'the target without ever tapping the currency dropdown)',
    (tester) async {
      await repo.createTrip(
        Trip(
          id: 't1',
      ...
      await repo.setExchangeRate(
        't1',
        const ExchangeRate(fromCurrency: 'CHF', toCurrency: 'EUR', rate: 1.05),
      );
      await repo.addExpense(
        Expense(
          id: 'e-eur',
      ...
      expect(
        find.byKey(const Key('directRateField_CHF')),
        findsNothing,
  ...
  testWidgets(
    'the category legend follows the currency switcher, not just the summary card',
    (tester) async {
      await repo.createTrip(
        Trip(
          id: 't1',
      ...
      await tester.pumpAndSettle();
      expect(
        find.textContaining('CNY 100.00'),
        findsWidgets,
        reason: 'legend defaults to home currency',
      );
      ...
      expect(
        find.textContaining('JPY 2,000.00'),
        findsWidgets,
        reason:
            'legend must follow the same view-currency switch as the summary card '
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      await tester.enterText(
        find.byKey(const Key('directRateField_CNY')),
        '0.05',
      );
      await tester.tap(find.byKey(const Key('confirmChangeCurrencyButton')));
      await tester.pumpAndSettle();
  ...
  testWidgets(
    'swiping an expense and confirming deletes it and refreshes the totals',
    (tester) async {
      await repo.createTrip(
        Trip(
          id: 't1',
      ...
      await tester.pumpAndSettle();
      expect(
        find.textContaining('CNY 300.00'),
        findsNothing,
        reason:
            'the Actual total must drop back to zero once the expense is gone',
      );
    },
  );

  testWidgets('cancelling the swipe-to-delete confirmation keeps the expense', (
    tester,
  ) async {
    await repo.createTrip(
      Trip(
  ...
  testWidgets(
    'deleting a trip removes it, its expenses, and pops back to the caller',
    (tester) async {
      await repo.createTrip(
        Trip(
          id: 't1',
              ...
              return ElevatedButton(
                onPressed: () async {
                  await Navigator.push<bool>(
                    ...
                    MaterialPageRoute(
                      builder: (_) =>
                          TripDetailScreen(tripId: 't1', repository: repo),
                    ),
                  );
                },
  ...
  testWidgets('cancelling the delete-trip confirmation keeps the trip', (
    tester,
  ) async {
    await repo.createTrip(
      Trip(
  ...
  testWidgets(
    'the CSV export button shares a file containing one row per expense',
    (tester) async {
      final trip = Trip(
        id: 't1',
        name: 'Japan',
